...
public class Instantiation implements CommandLineRunner{
	...
	public void run(String... args) throws Exception {
		...
		CommentDTO c1 = new CommentDTO("Boa Viagem mano", sdf.parse("27/03/2020"), new AuthorDTO(alex));
		CommentDTO c2 = new CommentDTO("Aproveite", sdf.parse("27/03/2020"), new AuthorDTO(bob));
		CommentDTO c3 = new CommentDTO("Tenha um ótimo dia!", sdf.parse("27/03/2020"), new AuthorDTO(alex));
		
		p1.getCommensts().addAll(Arrays.asList(c1, c2));
		p2.getCommensts().addAll(Arrays.asList(c3));
